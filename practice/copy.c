...
{   
    ...
    /* -rw-rw-r-- */
    /* permission -rwxrwxr-x |3 digi userpermission|3 digi grouppermission|3digi otherpermission| r-read w-write x-execution */

    /* read from source file and write to destination */
    /* read the first chunk */
    int numRead, numWritten; // numRead == numWritten to avoid partial write without awareness, because memory not enough
    numRead = read(sourceFd, buffer, CHUNK_SIZE);

    while(numRead  > 0) 
    {
        /* Write this chunk to the destination */
        numWritten = write(destinationFd, buffer, numRead); // 0, 1, 2 has special meaning to replace destinationFd
